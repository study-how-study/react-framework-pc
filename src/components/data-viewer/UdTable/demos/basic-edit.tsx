    ...
    <div>
      <UdTable
        ...
        columns={[
          ...
          {
            title: '性别', dataIndex: 'gender', render: (value) => value == 0 ? '女' : '男', editable: true, editor: (
              <Select>
          ...
          { title: '年龄', dataIndex: 'age', style: (text) => { return { color: text > 18 ? 'red' : 'greed' } } },
          {
            title: '操作', dataIndex: 'operate', render: (text, row, index) => {
              return <>
                <a onClick={() => {
                  const xxx = table.current.getRowsByKeys([row.udRowKey])
                  console.log(xxx)
                }}>获取行</a>
                &nbsp;
                <a onClick={() => {
                  table.current.setRowsByKeys({ age: 99 }, [row.udRowKey])
                }}>设置行</a>
              </>
            }
          },
